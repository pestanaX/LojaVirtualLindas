...
<?php
      ...
      include_once("/var/www/html/loja/Produto/ProdutoDao.php");     
      include_once("/var/www/html/loja/Produto/visualizacao.php");
      //include_once("/var/www/html/loja/Produto/Utilitario.php");       
  ...
  <div class= "row">
     ...
     <?php
        ...
        if (isset($_POST['pesquisa'])) {
         ...
         } else if ($produtoDao->numeroDeProdutos()<=9){
           for ($i =0;$i<count($todosProduto);$i++){                   
                $visual->imagemPrincipal($todosProduto[$i]['id_produto']);                       
          }   
         }else if ($produtoDao->numeroDeProdutos()>10){
            for ($i =0;$i<count($mostra);$i++){                   
                $visual->imagemPrincipal($mostra[$i]['id_produto']);                       
          }   
         }
...
<nav aria-label="Page navigation example">
  <ul class="pagination justify-content-center">
     ...
     <?php 
        $numeroPagina =intdiv ($produtoDao->numeroDeProdutos(),10);
       
       if ($produtoDao->numeroDeProdutos()>10) {
          for ($n =1;$n<=$numeroPagina+1;$n++) {           
              echo '<li class="page-item"> ';
              echo ' <a class="page-link" href=index.php?var='.$n.'>'.$n.'</a>';           
              echo '</li>';
           } 
       }
     ?>
